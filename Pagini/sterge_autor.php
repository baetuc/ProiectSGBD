...
<html>
  ...
  $stmt = oci_parse($connection, 'BEGIN CRUD.sterge_autor(:autID); END;');
  if (!$stmt) {
  ...
  oci_bind_by_name($stmt, ':autID', $_GET['autID']);
  ...
  $pagina_finala = $inceput . '<div><a href = "pagina_principala.php"> Înapoi</a></div>' . '<h3> Autor șters cu succes. </h3>';
